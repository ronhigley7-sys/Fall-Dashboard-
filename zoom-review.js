...
(function(){
  ...
  function staffYtdCount(name){
    ...
    return allData.filter(d=>{
      if(!d||!d._d||d._d.getFullYear()!==y||d._d>now) return false;
      return normStaffName(d.primaryRN)===target || normStaffName(d.primaryUAP)===target;
    }).length;
  }
  ...
  function render(arg){
    ...
    document.getElementById('zoomReviewSubtitle').textContent=`Grouped by unit, then date · ${typeof fUnit!=='undefined'&&fUnit!=='ALL'?fUnit:'All units'} · ${typeof fMonth!=='undefined'&&fMonth!=='ALL'?fMonth:'All months'}`;
