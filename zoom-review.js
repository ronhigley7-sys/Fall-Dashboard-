...
(function(){
  ...
  const unitOrder=['3rd Floor','4B','4D','5D','ED','ICU'];

  function rows(){
    if(typeof getF!=='function') return [];
    return getF().slice().sort((a,b)=>{
      const au=a.unit||'';
      const bu=b.unit||'';
      const ai=unitOrder.includes(au)?unitOrder.indexOf(au):unitOrder.length;
      const bi=unitOrder.includes(bu)?unitOrder.indexOf(bu):unitOrder.length;
      if(ai!==bi) return ai-bi;
      if(au!==bu) return au.localeCompare(bu);
      const ad=(a._d&&a._d.getTime())||0;
      const bd=(b._d&&b._d.getTime())||0;
      if(ad!==bd) return ad-bd;
      return String(a.eventTime||'').localeCompare(String(b.eventTime||''));
    });
  }
  function key(r){return [r.facility||'',r.unit||'',String(r.eventDate||''),String(r.eventTime||'')].join('|').toLowerCase().trim();}
  function normStaffName(n){return (n||'').toString().trim().toLowerCase().replace(/\s+/g,' ');}
  function noCANoted(name){
    const n=normStaffName(name).replace(/[._-]+/g,' ').replace(/\s+/g,' ').trim();
    return !n || n==='of form' || n==='off form' || n==='on form' || n==='form';
  }
  function staffYtdCount(name){
    const target=normStaffName(name);
    if(!target||!Array.isArray(allData)) return 0;
    const now=new Date(), y=now.getFullYear();
    return allData.filter(d=>d&&d._d&&d._d.getFullYear()===y&&d._d<=now&&(normStaffName(d.primaryRN)===target||normStaffName(d.primaryUAP)===target)).length;
  }
  function staffLabel(role,name){
    if(role==='UAP/CA'&&noCANoted(name)) return 'No CA noted';
    if(!name) return `${role}: —`;
  ...
  function ensureUI(){
    ...
    if(incBtn){const b=document.createElement('button');b.className='tab-btn';b.id='zoomTabBtn';b.textContent='🎥 Zoom Review';b.onclick=function(){showTab('zoomreview',b);};incBtn.insertAdjacentElement('afterend',b);}
    const content=document.getElementById('content');
    if(content){
      const pane=document.createElement('div');pane.id='tab-zoomreview';pane.className='tab-pane';
      pane.innerHTML=`<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:14px"><div><div style="font-weight:800;font-size:18px;color:#1B2A4A">Fall Incident Zoom Review</div><div id="zoomReviewSubtitle" style="font-size:12px;color:#5E6E8C;margin-top:3px"></div></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn-teal" id="zoomPrevBtn">← Previous</button><button class="btn-teal" id="zoomNextBtn">Next →</button><button class="btn-red" id="zoomResetBtn">Reset Reviewed</button></div></div><div id="zoomReviewKPIs" style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:14px"></div><div class="card" style="padding:10px 14px"><div style="display:flex;align-items:center;gap:10px"><div style="font-size:10px;font-weight:800;text-transform:uppercase;color:#5E6E8C;white-space:nowrap">Meeting Progress</div><div class="bar-track" style="flex:1;height:10px;margin:0"><div id="zoomProgressBar" class="bar-fill" style="width:0%;background:#0E7C7B"></div></div><div id="zoomProgressText" style="font-size:12px;font-weight:800;white-space:nowrap">0 / 0 reviewed</div></div></div><div id="zoomReviewCard"></div><div id="zoomReviewQueue" class="card" style="margin-top:14px"></div>`;
      const upload=document.getElementById('tab-upload');if(upload)content.insertBefore(pane,upload);else content.appendChild(pane);
      document.getElementById('zoomPrevBtn').onclick=prev;document.getElementById('zoomNextBtn').onclick=next;document.getElementById('zoomResetBtn').onclick=function(){reviewed.clear();reviewIndex=0;render();};
    }
  }
  ...
  function render(arg){
    ensureUI();const r=Array.isArray(arg)?arg:rows();const card=document.getElementById('zoomReviewCard'),queue=document.getElementById('zoomReviewQueue'),kpis=document.getElementById('zoomReviewKPIs');if(!card||!queue||!kpis)return;if(reviewIndex>=r.length)reviewIndex=Math.max(0,r.length-1);
    const done=r.filter(x=>reviewed.has(key(x))).length,injuries=r.filter(x=>['Minor','Moderate','Major'].includes(x.injuryLevel)).length,huddles=r.filter(x=>(x.huddleCompleted||'').toString().toLowerCase().includes('yes')).length,unassisted=r.filter(x=>{const v=(x.assisted||'').toString().toLowerCase();return v.includes('no')||v.includes('unassist');}).length;
  ...
  function install(){ensureUI();if(typeof showTab==='function'&&!showTab._zoomWrapped){const base=showTab;window.showTab=function(id,btn){base(id,btn);if(id==='zoomreview')render();};window.showTab._zoomWrapped=true;}if(typeof applyFilters==='function'&&!applyFilters._zoomWrapped){const base=applyFilters;window.applyFilters=function(){base();if(document.getElementById('tab-zoomreview')?.classList.contains('active'))render();};window.applyFilters._zoomWrapped=true;}if(typeof setUnitFilter==='function'&&!setUnitFilter._zoomWrapped){const base=setUnitFilter;window.setUnitFilter=function(u){base(u);if(document.getElementById('tab-zoomreview')?.classList.contains('active'))render();};window.setUnitFilter._zoomWrapped=true;}render();}
